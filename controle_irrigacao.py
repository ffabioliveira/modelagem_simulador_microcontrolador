...
class ControleIrrigacao:
    ...
    def publicar_volume_periodicamente(self, tempo_acionamento, intervalo, ciclo_total):
        self.ciclo_total = ciclo_total
        while self.dia_atual <= self.ciclo_total:
            ...
            self.dia_atual += 1
            print(f"Dia {self.dia_atual} do ciclo da cultura iniciado.")
            self.valvula.abrir()
    ...
    def processar_mensagem(self, client, userdata, message):
        ...
        if "Ciclo:" in mensagem_recebida:
            ...
            self.sensor.iniciar_medicao()
            self.publicar_volume = True
            threading.Thread(target=self.publicar_volume_periodicamente, args=(tempo_acionamento, intervalo, ciclo_total)).start()
